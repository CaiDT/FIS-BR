...
class AttentionSpatiotemporalBlock(nn.Module):
    ...
    def forward(self, x):
        # temporal block before attention vector
        x_t = self.group_conv_temporal(x)  # 先经过一个3D卷积
        x_t = self.bn_temporal(x_t)   # 批量归一化
        x_t = self.relu(x_t)  # 激活函数
        ...
        if network.REC:
            network.TemporalAttention_List.append(attention_t)
            network.SpatialAttention_List.append(attention_s)
